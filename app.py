...
def monitor_co():
    ...
    data = pd.read_csv("./incendiosAD/Sentinel-5P.csv")
    ...
    plt.savefig("./templates/grafica.jpg", format='jpg')
    plt.close()  

    # Guardar la imagen en un buffer y convertir a base64
    buffer = io.BytesIO()
...
def detect_fire(cap):
    ...
    # Definición de rangos de color para detectar fuego
    ...
    while True:
        ...
        # Convertir a espacio de color HSV y crear las máscaras
        hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        ...
        for contour in contours:
            ...
            if area > 400:  # Filtrar objetos pequeños
                cv2.drawContours(frame_copy, [contour], -1, (255, 105, 180), -1)  # Color para contornos

        # Agregar la imagen procesada a la lista
        processed_images.append(frame_copy)

    # Convertir a GIF y base64
    if processed_images:
        gif_path = './templates/filtro.gif'
        ...
        with open(gif_path, "rb") as gif_file:
            img_fire_base64 = base64.b64encode(gif_file.read()).decode('utf-8')
        
        return img_fire_base64
    else:
        print("No se procesaron imágenes de detección de incendios.")
        return None
...
def incendios():
    ...
    gif_to_process = cv2.VideoCapture('./incendiosAD/calor.gif')  # Cargar el GIF
    gif = detect_fire(gif_to_process)

    if gif is None:
        return jsonify({"error": "No se pudo generar el GIF de incendios."})
